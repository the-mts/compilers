...
void BFS(struct vertex **adj,int * parent, int* weight, int* status,int n,int v1,int v2){
    ...
    int queue[10001];
    // int *queue;
    // queue = (int*) malloc(10001*(sizeof(int)));
    queue[tail++]=v1;
    status[v1]=1;
    while(head!=tail){
...
int find(struct node *xx, struct node* set){
    struct node x = *xx;
    struct node y = x;
    int result;
...
void MergeSort(struct edge* arr,int p,int r){
    ...
    if(p<r){
        ...
        // printf("Merg/e between 1 %d %d %d\n", p,q,  r );
        MergeSort(arr,p,q);
        
        // printf("Merge between 2 %d %d %d\n", p,q,  r );
        MergeSort(arr,q+1,r);
        
        // printf("Merge between 3 %d %d %d\n", p,q,  r );
        Merge(arr,p,q,r);
        
        
    }
    else {
    // printf("Merge bye %d %d\n", p, r );
        return ;
    }
    // printf("Merge bye %d %d\n", p, r );
}
