...
$(SRC)/y.tab.c $(SRC)/y.tab.h: $(SRC)/parser.y $(SRC)/parse_utils.h $(SRC)/3AC.h
	$(YACC) -dvt -Wno-yacc -Wno-other -o $(SRC)/y.tab.c $<

$(BIN)/main.o: $(SRC)/main.c $(SRC)/parse_utils.h $(SRC)/symtab.h $(SRC)/3AC.h $(SRC)/codegen.h
	$(CC) -c $< -o $@

$(BIN)/parse_utils.o: $(SRC)/parse_utils.c $(SRC)/parse_utils.h $(SRC)/symtab.h
	$(CC) -c $< -o $@

$(BIN)/symtab_utils.o: $(SRC)/symtab_utils.c $(SRC)/symtab.h
	$(CC) -c -Wno-abi $< -o $@

$(BIN)/3AC.o: $(SRC)/3AC.c $(SRC)/3AC.h $(SRC)/symtab.h $(SRC)/parse_utils.h
	$(CC) -c $< -o $@

$(BIN)/codegen.o: $(SRC)/codegen.c $(SRC)/codegen.h $(SRC)/3AC.h $(SRC)/symtab.h
	$(CC) -c $< -o $@
